...
//'?.'을 기준으로 앞의 평가대상이 undefind || null 이 아니면 처리를 계속 이어가고
...
//push(...args)
//원본 배열에 마지막 요소를 추가하고,바뀐 length 반환 
const resultLengh = arr4.push(100);
console.log( resultLengh, arr4);
...
const arr6 = [1,2,3];
const resultPop = arr6.pop(arr6.pop);
console.log(arr6,resultPop);
arr6.pop();

//unshift(...args) : 원본 배열의 첫번 째 요소를 추가,변경된 length를 반환
const arr7 = [1,2];
const resultUnshift = arr7.unshift(2,3,4);
console.log(arr7, resultUnshift);

//shift() : 원본 배열의 첫번 째 요소를 제거하고, 제거된 요소를 반환
...
// start만 지정 했을 때
 //start가 양수일 경우, 인덱스가 start인 요소부터 모두 제거하고 잘린 배열을 반환한다 
 ...
 //(a-b)가 음수일 경우, a가 작은 수 , b가 큰수로 인식하여 정렬 
 ...
 // map(callback) : 배열의 모든요소에 대해 콜백함수를 반복하여 실행 한 후, 새로운 배열을 반환
...
//2의 배수만 '이'출력해 주세요
 ...
 console.log(arr15, resultArr);

 // some(cd) : 배열의 모든 요소에 대해서 콜백함수를 반복 실행 후,
 // 조건에 만족하는 결과가 하나라도 있으면 true, 없으면 false
 const arr16 = [1, 2, 3, 4, 5];
 const resultSome= arr16.some(val =>val === 10);

 //every(cb) 배열의 모든 요소에 대해서 콜백 함수를 실행 후 ,
 //모든요소가 조건에 만족하면 true,아니면 false
 ...
 //forwach(cb) : 배열의 모든 요소애 대해 콜백함수를 반복 실행
